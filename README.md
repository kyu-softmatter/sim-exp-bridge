...
실제로 일어났다. `4652273`(내 커밋)이 r2 에 `gaps[].kind` 를 붙였다 — 내용은 쓰인 대로
보존했지만 해시가 움직였다. r8 이 깨졌다. 접미사 없는 `bd:` 키를 갱신했더니 r2 를
인용하는 r4 가 깨졌다. r4 를 갱신했더니 r4 를 인용하는 r5 와 r7 이 깨졌다. 세 번째에서
형태가 드러났다.
