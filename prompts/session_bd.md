# 이 세션(Brownian-Dynamics-Agent)에 붙여넣을 요청문
...
이 리포(Brownian-Dynamics-Agent) 안에서만 작업한다. **현재 체크아웃되어 있는
브랜치 그대로 작업하고, 브랜치를 바꾸거나 새로 만들지 않는다** — 어느 브랜치인지는
이 세션이 이미 알고 있고, 사람이 그 브랜치를 고른 이유가 있다. 워크트리 안에서
돌고 있다면 그 워크트리가 작업 위치다.

**AM 리포(`~/Desktop/agentic-microscope`)의 파일은 읽기만 하고 절대 수정하지 않는다.** AM의 `CLAUDE.md`도 로드하지 않는다 —
